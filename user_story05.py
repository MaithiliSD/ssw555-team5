...
def MarriageBeforeDeath(fam_list, indi_list):
    death_data = []
    for i in fam_list:
        if(deathDate(indi_list, i[1]) != None):
            if(i[3] > deathDate(indi_list, i[1])):
                death_data.append(i[0])
                print("The family " + i[0] + " has marriage date after death date of " + i[1])
        if(deathDate(indi_list, i[2]) != None):
            if(i[3] > deathDate(indi_list, i[2])):
                death_data.append(i[0])
                print("The family " + i[0] + " has marriage date after death date of " + i[2])
    if(len(death_data) == 0):
        print("No Families had thier marriage after death date ")
    else:
        print("Families have marriage date after death date ")
        print(death_data)
