...
def name_by_id(indi_list, id):
    '''Getting the name of the individual by ID'''
    for i in indi_list:
        if(i[0] == id):
            return i[1]
...
def deceased_list(indi_list):
    '''Printing the deceased list'''
    death_list = []
    for individual in indi_list:
        if individual[4] != 0:
            death_list.append(individual[0])
